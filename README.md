...
[![DeepSeek](https://img.shields.io/badge/DeepSeek-V4-blueviolet?style=for-the-badge)](https://deepseek.com)
...
4. **Provides Arabic-language explanations** generated by DeepSeek-V4, strictly grounded in the actual data and the actual decision — never fabricating outcomes.
...
│                                                         │
│   DeepSeek-V4 via OpenRouter → Arabic explanation only  │
...
sequenceDiagram
    ...
    participant LLM as DeepSeek-V4 (OpenRouter)
...
*Note on architectural scope: Tamweel utilizes two distinct knowledge grounding approaches:*
*- The conversational **chat assistant** uses the full hybrid vector search pipeline described below.*
*- The **credit-decision explanation engine** (`rag_engine.py`) uses a statically defined, rule-based dictionary of curated financial context rather than live vector retrieval to ensure absolute determinism during scoring.*
...
| LLM (Explanation) | DeepSeek-V4 via OpenRouter | Arabic credit decision explanation |
...
| Embeddings | OpenAI text-embedding-3-small via OpenRouter | RAG vector search |
...
│   ├── rag_engine.py            # DeepSeek explanation engine + score firewall
...
# AI Models
OPENROUTER_API_KEY=<your-openrouter-key>       # DeepSeek-V4 (explanation) + GPT-4o-mini + embeddings
